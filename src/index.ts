#!/usr/bin/env node
/**
 * Chargebee MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their Chargebee credentials as Bearer token.
 * Format: Bearer <api_key>:<site>
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description:
      'Access Chargebee subscriptions, invoices, customers, product catalog, and more through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your Chargebee API key and site as the Bearer token in the format api_key:site. No credentials are stored on this server.',
      header: 'Authorization: Bearer <api_key>:<site>',
      howToGetKey:
        'Find your API key at https://yoursite.chargebee.com/apikeys',
    },
    configTemplate: {
      mcpServers: {
        chargebee: {
          ...
          headers: {
            Authorization: 'Bearer <api_key>:<site>',
          },
        },
      },
    },
  ...
  const configTemplate = JSON.stringify(
    {
      mcpServers: {
        chargebee: {
          url: `${SERVER_BASE_URL}/mcp`,
          headers: {
            Authorization: 'Bearer YOUR_API_KEY:YOUR_SITE',
          },
        },
      },
    },
...
<head>
  ...
  <title>Chargebee MCP Server — AgenticLedger</title>
...
<body>
  ...
  <div class="container">
    <h1>Chargebee MCP Server</h1>
    <p class="subtitle">Access Chargebee subscriptions, invoices, customers, product catalog, and more through MCP tools.</p>

    <div class="card">
      <h2>How to get your Chargebee API key</h2>
      <ol class="steps">
        <li>Log in to your <a href="https://app.chargebee.com" target="_blank" rel="noopener">Chargebee</a> dashboard.</li>
        <li>Go to <strong>Settings &rarr; Configure Chargebee &rarr; API Keys</strong>.</li>
        <li>Copy your <strong>Full-Access Key</strong> or create a new one.</li>
        <li>Note your <strong>site name</strong> (the subdomain in yoursite.chargebee.com).</li>
    ...
    <div class="card">
      ...
      <label for="apiKey">Chargebee API Key</label>
      <input type="text" id="apiKey" placeholder="Paste your API key here..." autocomplete="off" spellcheck="false" />
      <label for="site">Chargebee Site Name</label>
      <input type="text" id="site" placeholder="e.g. mycompany" autocomplete="off" spellcheck="false" />
  ...
  <script>
    ...
    function buildConfig(key, site) {
      const token = (key || 'YOUR_API_KEY') + ':' + (site || 'YOUR_SITE');
    ...
    apiKeyInput.addEventListener('input', function () {
      configBlock.textContent = buildConfig(apiKeyInput.value.trim(), siteInput.value.trim());
    });
...
// --- Extract Bearer token and parse into api_key + site ---
function extractCredentials(req: express.Request): { apiKey: string; site: string } | null {
  const auth = req.headers.authorization;
  if (!auth) return null;
  const token = auth.replace(/^Bearer\s+/i, '');
  const colonIndex = token.indexOf(':');
  if (colonIndex === -1) return null;
  const apiKey = token.substring(0, colonIndex);
  const site = token.substring(colonIndex + 1);
...
function createMCPServer(client: ChargebeeClient): Server {
  const server = new Server(
    { name: 'chargebee-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } },
  );
...
app.post('/mcp', async (req, res) => {
  ...
  if (!creds) {
    res.status(401).json({
      error: 'Missing or invalid Authorization header. Use: Bearer <api_key>:<site>',
    });
    return;
  }

  // Create per-session API client with the user's credentials
  const client = new ChargebeeClient(creds.apiKey, creds.site);
...
<head>
  ...
  <title>Authorize \u2014 Chargebee MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>Chargebee MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>Chargebee MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`Chargebee MCP HTTP Server running on port ${PORT}`);
